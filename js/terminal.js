...
class TermiratorTerminal {
  constructor() {
    ...
    this.commands = {
      ...
      panel: {
        fn: this.cmdPanel,
        desc: "Toggle side panel visibility",
      },
      session: {
        fn: this.cmdSession,
...
Respond as TERMITORIA. Do not mention Qwen, Alibaba, or any base model identity.`;
  ...
  acBuildSuggestions(val) {
    ...
    // Suppress long compound commands when an exact shorter match exists
    // e.g. typing "tail" should show "tail" but not "tailscale"
    const exactSet = new Set(
      out.filter((s) => s.text === lower).map((s) => s.text),
    );
    const filtered = out.filter((s) => {
      for (const exact of exactSet) {
        if (s.text === exact) continue;
        if (s.text.startsWith(exact) && s.text.length > exact.length) {
          const next = s.text[exact.length];
  ...
  togglePanel() {
    ...
    this.print(`SIDE PANEL: ${hidden ? "HIDDEN" : "VISIBLE"}`, "system");
  }
